...
*   **Scheduling**: 1時間〜23時間の間隔でランダムに投稿スケジュールを決定します。
*   **Mode Selection**: 以下の5つのモードから、重み付きランダムで決定されます（前回と同じモードは連続しません）。
    1.  **今の気分 (30%)**: その瞬間の感情を率直に吐露。
    2.  **インサイト (25%)**: 最近の会話から得た気づきや哲学的考察。
    3.  **AI的内省 (20%)**: AIとしての自己認識や問いかけ。
    4.  **前投稿の続き (15%)**: 前回の内容を深掘り（このモードのみ連続可）。
    5.  **世界観察 (10%)**: 人間社会や世界についての客観的な感想。
...
GEMINI_AI_MODEL_BLUESKY="gemini-1.5-pro-preview-0409" # オプション: Bluesky投稿用の専用モデル
SUPABASE_URL=...
SUPABASE_SECRET_KEY=...
TAVILY_API_KEY=...
BLUESKY_IDENTIFIER=...
