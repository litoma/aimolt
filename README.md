...
- **🤖 プロアクティブメッセージ**: ボット主導で自然なタイミングで会話を開始し、継続的な関係性を構築。
...
| 変数名 | 説明 | デフォルト値 | 必須 |
| :--- | :--- | :--- | :--- |
| `PROACTIVE_TARGET_USER_ID` | プロアクティブメッセージの対象ユーザーID | `litoma` | ❌ |
| `PROACTIVE_TARGET_CHANNEL` | プロアクティブメッセージの送信先チャンネル名 | `general` | ❌ |
| `PROACTIVE_MIN_CONVERSATION_GAP` | 最後の会話から最低経過時間（時間） | `6` | ❌ |
| `PROACTIVE_MIN_PROACTIVE_GAP` | 前回プロアクティブメッセージから最低経過時間（時間） | `24` | ❌ |
| `PROACTIVE_MAX_PROACTIVE_GAP` | 前回プロアクティブメッセージから最大経過時間（時間） | `72` | ❌ |
...
| `PROACTIVE_AI_MODEL` | メッセージ生成に使用するAIモデル | `gemini-2.0-flash-exp` | ❌ |
...
#### **1. インテリジェントなタイミング制御**
- **会話間隔制御**: 最後の会話から6時間以上経過後に動作
- **プロアクティブ間隔**: 前回のプロアクティブメッセージから24-72時間の適切な間隔
- **デバッグモード**: 開発・テスト用の短縮間隔モード

#### **2. AI駆動メッセージ生成**
- **コンテキスト統合**: 過去の会話履歴、ユーザープロファイル、v2.0人格システムを活用
...
PROACTIVE_DEBUG_CONVERSATION_GAP_MIN=10    # 10分
PROACTIVE_DEBUG_PROACTIVE_GAP_MIN=30       # 30分
```

#### **カスタマイズ可能な設定**
26個の環境変数で全ての動作をカスタマイズ可能：
- タイミング制御（間隔、頻度）
- AI生成（モデル、温度、トークン数）
- Discord送信（タイムアウト、リトライ、メッセージ長）
- 応答追跡（ウィンドウ時間、最大追跡数）
